const service = require('./procedure-referral.service');
...
async function list(req, res, next) {
  try {
    ...
    res.json(await withDoctorNome(req.tenantId, items));
  } catch (err) { next(err); }
}
...
async function getOne(req, res, next) {
  try {
    ...
    if (!item) return res.status(404).json({ error: 'Encaminhamento de procedimentos nao encontrado(a).' });
...
async function update(req, res, next) {
  try {
    ...
    if (!item) return res.status(404).json({ error: 'Encaminhamento de procedimentos nao encontrado(a) ou sem permissao.' });
    res.json(item);
  } catch (err) { next(err); }
}
