...
const WRITABLE = ['paciente', 'medicamento', 'nivel_prioridade', 'setor_destino'];
...
async function findAll(tenantId, requester) {
  return prisma.medicationReferral.findMany({ where: scopedFilter(tenantId, requester), orderBy: { createdAt: 'desc' } });
}

async function findById(tenantId, id, requester) {
  if (!isValidUuid(id)) return null;
  return prisma.medicationReferral.findFirst({ where: { id, ...scopedFilter(tenantId, requester) } });
}
...
async function create(tenantId, data, requester) {
  ...
  return prisma.medicationReferral.create({ data: { ...safeData, tenantId, doctorId } });
}
...
async function update(tenantId, id, data, requester) {
  ...
  const existing = await prisma.medicationReferral.findFirst({ where: { id, ...scopedFilter(tenantId, requester) } });
  if (!existing) return null;
  return prisma.medicationReferral.update({ where: { id }, data: safeData });
}

async function remove(tenantId, id) {
  if (!isValidUuid(id)) return;
  await prisma.medicationReferral.deleteMany({ where: { id, tenantId } });
}
